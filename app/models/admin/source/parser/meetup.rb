class Admin::Source::Parser::Meetup < Admin::Source::Parser
  self.label = :Meetup
  self.url_pattern = %r{^http://(?:www\.)?meetup\.com/[^/]+/events/([^/]+)/?}

  def to_events
    return fallback unless Rails.application.secrets.meetup_api_key.present?
  ...
  def get_data
    to_events_api_helper(opts[:url], "problem") do |event_id|
      [
        ...
        {
          key: Rails.application.secrets.meetup_api_key,
          sign: 'true'
        }
      ]
    end
